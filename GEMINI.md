# TWISTED — The Glass Engine (Decision Engine)

## Project Overview

TWISTED (also referred to as Decision Engine orgaNICE) is a local-first, automated decision-engine application designed to process files and generate strategic recommendations for specific beneficiaries. It uses a Multi-Agent AI System (Context Analyzer, Legal Advisor, Strategist, Final Reviewer) driven by local or cloud LLMs (Google Gemini, OpenAI, LMStudio, Apple MLX).

The system architecture consists of:
- **Backend:** Python-based FastAPI server managing the multi-agent orchestration, ingestion pipeline, ChromaDB vector memory, and WebSocket real-time communication.
- **Frontend:** A React web application (in `frontend/`) using Vite, and potentially a SwiftUI application (in `DecisionEngine/`).
- **Agents:** Configurable agent personas stored in the `agents/` directory using Markdown profiles (`IDENTITY.md`, `SKILLS.md`, `SOUL.md`).

## Building and Running

### Backend Setup (Python)
The backend requires Python 3 and uses a virtual environment.
...
**Configuration:**
This project separates application secrets from build/AI agent secrets:
1. **Application Secrets (`.env`)**: Create a `.env` file based on `backend/.env.example` and add your API keys (e.g., `GEMINI_API_KEY`, `OPENAI_API_KEY`). These are used *only* by the running application.
2. **Build/Agent Secrets (`.env.build`)**: Create a `.env.build` file based on `.env.build.example` in the root. This file is for AI build agents and the development process. It contains GitHub PATs, URLs to alternative frontends (e.g., the one created in Google AI Studio), and the alternative full app version (https://agent.ii.inc/). Building agents **must** use these credentials when interacting with external repos or fetching cherry-picked features.
...
The web frontend is a React 19 application built with Vite.

```bash
cd /Users/perbrinell/Documents/orgaNICE2/frontend
npm install
npm run dev
```

### Docker Deployment
A production-ready stack is available via Docker Compose, which spins up the FastAPI backend, ChromaDB, Nginx frontend, and Redis.
...
- **Multi-Agent System:** Agents debate and analyze cases. Their personas are entirely configurable via the `agents/<agent_name>/` directories. When modifying agent behavior, update their respective Markdown profiles (`IDENTITY.md`, `SKILLS.md`, `SOUL.md`).
- **WebSocket Communication:** The backend heavily relies on WebSockets (`/ws/cases/{case_id}`) to stream real-time progress, agent thoughts, event logs, and audio transcripts to the frontend.
- **Vector Memory:** ChromaDB is used for semantic search and storing external intel during deep research phases.
- **Ingestion:** The system supports a wide array of file types (Text, PDF, DOCX, Email, Images, Video, Audio). 
- **LLM Routing:** A `HybridLLMRouter` is used to manage requests between local models (LMStudio) and cloud models (Gemini) based on availability and resources.

### Internal Agent Resources
When working on the engine's core features or agent behaviors, building agents **must** consult the following files for context:
1. `orgaNICE_v3_one_shot_prompt.md`: The absolute master blueprint for the entire application (previously named orgaNICE, now TWISTED). Contains architecture, deep research logic, and UI standards ("Liquid Glass").
2. `kchat.txt`: The definitive architectural chat log (Chunks 1-8). It contains the exact implementations of the Swarm Debate Engine, M4 Resource Guardian, Glassmorphism CSS physics, and Custom Tools system.
3. `test_case_alfa_corp.txt`: Sample test case data intended to be parsed and debated by the swarm.
